...
const httpCentroFormacion = {
  ...
  getCentroFormacion: async (req, res) => {
    const centrosFormacion = await CentroFormacion.find().populate("idCiudad");
  ...
  postCentroFormacion: async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ mensaje: "Error al crear el centro de formación" });
    }
  },
  // editar usuario
  putCentroFormacion: async (req, res) => {
    ...
    try {
      ...
      if (!centroFormacion) {
        return res
          .status(404)
          .json({ error: "Centro de formación no encontrado" });
      }
      ...
      res
        ...
        .json({
          error:
            "Error en el servidor a la hora de editar el centro de fromación",});
    }
  },
